...
void copiazaCaseScumpe(struct Casa* vector, char nrElemente, float pretMinim, struct Casa** vectorNou, int* dimensiune) {
    *dimensiune = 0;
    ...
    // daca era deja alocat, il dezalocam corect
    if (*vectorNou != NULL) {
        int dimVeche = *dimensiune; // nu o stim, deci NU putem dezaloca corect aici fara dim.
        // Ca in laborator02, facem free simplu (merge daca n-ai mai avut deep copy),
        // dar corect e sa apelezi dezalocare daca stii dim veche.
        free(*vectorNou);
        *vectorNou = NULL;
    }
...
struct Casa getPrimaCasaByStrada(struct Casa* vector, int nrElemente, const char* strada) {
    ...
    return c; // daca nu gaseste, returneaza c cu strada = NULL
}
