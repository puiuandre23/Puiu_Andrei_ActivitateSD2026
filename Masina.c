...
struct Masina {
    int id;
    int nrUsi;
    char* model;     // char* obligatoriu ca in sablon
    float pret;
    char serie;
};
...
// similar cu sablon: copiazaAnumiteElemente(vector, nrElemente, prag, &vectorNou, &dimensiune)
void copiazaMasiniScumpe(struct Masina* vector, char nrElemente, float pretMinim,
    ...
    *dimensiune = 0;
    ...
    if (*vectorNou != NULL) {
        free(*vectorNou);     // ca in laborator02 (simplu)
        *vectorNou = NULL;
    }
...
// similar cu sablon: getPrimulElementConditionat(..., const char* conditie)
// aici conditia e "model"
